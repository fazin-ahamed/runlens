...
these are single-host observations, not a benchmark suite. they vary
with filesystem, CPU, output volume and which collectors are enabled.
treat everything here as order-of-magnitude guidance.
...
reading 4KB chunks from a pseudo terminal and redaction-scanning:

100MB of pure pseudo terminal output -> ~1.6s end-to-end (read + redact + seal)

## real workload example

for a typical `cargo test` recording (~3000 events, ~6MB pseudo terminal output):
