...
- **windows pseudo terminal** uses portable-pty's ConPTY backend. sometimes
  ConPTY drops the first byte of stdout on certain terminal widths.
  we force 120 cols to avoid it but your mileage may vary.
- **macOS** pseudo terminal support works but we dont test it regularly.
...
- **pseudo terminal signal handling is lossy.** portable-pty's ExitStatus has
  signal() as a private field so we only get exit codes.
- **pseudo terminal drain loop sleeps 25ms fixed.** fine for normal use but adds
...
- recorder has 12 unit tests but no integration test that spawns a
  real pseudo terminal child end-to-end. reason: ConPTY is flaky on the host we
