...
async fn export_and_import_roundtrip_preserves_chain() {
    ...
    repo_src.ensure_project(&project).unwrap();
    ...
    repo_src.create_session(&session).unwrap();
    ...
    for i in 0..10 {
        ...
        prev = new_hash;
        repo_src.append_event(&e).unwrap();
    }
    repo_src
    ...
    let events_after = repo_dst.list_events(&session_id).unwrap();
