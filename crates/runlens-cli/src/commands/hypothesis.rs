...
pub async fn run(_workspace: &WorkspacePaths, args: &HypothesisArgs) -> Result<()> {
    match &args.cmd {
        HypothesisCommand::Create { name, session } => {
            println!("Created workspace '{}' (session: {})", name, session.as_deref().unwrap_or("none"));
        }
        HypothesisCommand::Propose { workspace_id, description, author } => {
            println!("Proposed: '{}' in workspace {} by {}", description, workspace_id, author);
        }
        HypothesisCommand::List { workspace_id, status } => {
            println!("Hypotheses in {} (status: {}):", workspace_id, status.as_deref().unwrap_or("all"));
            println!("  (none)");
        }
