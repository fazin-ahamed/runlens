...
async fn handle_method(
    ...
    match method {
        ...
        protocol::methods::DAEMON_INGEST => {
            let payload = params
                .ok_or_else(|| JsonRpcError::invalid_params("missing event payload"))?;
            ...
            s.ingest.ingest(event).await.map_err(|_| {
                JsonRpcError::internal_error("ingest channel closed; daemon shutting down")
            })?;
        ...
        protocol::methods::SESSION_LIST => {
            let s = state.read().await;
            let sessions = s.repo.list_recent_sessions(100)
        ...
        protocol::methods::SESSION_GET => {
            ...
            let s = state.read().await;
            let session = s.repo.get_session(&session_id)
                .map_err(|e| JsonRpcError::internal_error(format!("session {session_id}: {e}")))?;
            let events = s.repo.list_events(&session_id)
        ...
        protocol::methods::GRAPH_TRACE => {
            ...
            let s = state.read().await;
            let graph = GraphBuilder::new(&s.repo).load(&trace_id)
        ...
        protocol::methods::GRAPH_COMPARE => {
            ...
            let s = state.read().await;
            let repo = &s.repo;
            let ga = GraphBuilder::new(repo).load_session(a)
                .map_err(|e| JsonRpcError::internal_error(format!("graph error: {e}")))?;
            let gb = GraphBuilder::new(repo).load_session(b)
                .map_err(|e| JsonRpcError::internal_error(format!("graph error: {e}")))?;
        ...
        protocol::methods::GRAPH_CHAIN => {
            let p = params.ok_or_else(|| JsonRpcError::invalid_params("missing params"))?;
            let trace_id = p.get("trace_id").and_then(|v| v.as_str())
                .ok_or_else(|| JsonRpcError::invalid_params("missing trace_id"))?;
            let span_id = p.get("span_id").and_then(|v| v.as_str())
        ...
        protocol::methods::QUERY_EXECUTE => {
            ...
            let rows = run_query(&conn_guard, &rql)
                .map_err(|e| JsonRpcError::internal_error(format!("query error: {e}")))?;
