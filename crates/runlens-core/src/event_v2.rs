...
// EventV2 is the wire model used by the daemon's live pipeline and by
// runlens-db's analysis. It is intentionally distinct from the v1 storage
// Event in crate::model: v1 is the on-disk event, while EventV2 carries the
// span/source-clock fields the daemon needs for streaming. Conversion between
// the two goes through EventV2::from_v1 (v2 -> v1 storage serialization is
// handled in runlens-storage). Keep the two in sync when adding fields.
//
// ponytail: no migration machinery beyond from_v1; add an explicit
// EventV2::from(event) reverse path if storage ever needs to persist v2 shape.
